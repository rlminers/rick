...
# start nmon
nmon -F $LOG_DIR/${PREFIX}.nmon -T -s 60 -c 120
...
# start charbench
nohup ${CWD}/charbench -c $CF -cs $CS -uc $UC -rt 02:00 -r $LOG_DIR/${PREFIX}-swingbench.xml -a -v users,tpm,tps,cpu > charbench.out &
...
getawr ${array1[6]}  ${array1[7]} $PREFIX ${INSTNUM} $LOG_DIR
getawr ${array1[7]}  ${array1[8]} $PREFIX ${INSTNUM} $LOG_DIR
getawr ${START_SNAP} ${array1[8]} $PREFIX ${INSTNUM} $LOG_DIR
